...
export interface Map2GISProps
  extends Partial<MapEventHandlerTable>,
    MapCustomEvents {
...
const Map2GISComponent: FC<Map2GISProps> = (props) => {
  ...
  const [forceReSubscribeEvents, setForceReSubscribeEvents] =
  ...
  const prevEventHandlersKeysRef = useRef<
    Array<keyof Partial<MapEventHandlerTable>>
  >([]);
  ...
  useEffect(() => {
    ...
    for (const currentKey in props) {
      if (
        ...
        !CUSTOM_EVENTS.includes(
          currentKey as keyof (MapCustomEvents & Partial<MapEventHandlerTable>)
        )
      ) {
    ...
    const handlersKeys = Object.keys(eventHandlersRef.current) as Array<
      keyof Partial<MapEventHandlerTable>
    >;
  ...
  useEffect(() => {
    ...
    const instance = new mapGLBundle.Map(
  ...
  const eventHandler = useCallback(
    (key: keyof MapEventHandlerTable) =>
      (event: MapEventTable[keyof MapEventTable]) => {
  ...
  useEffect(() => {
    ...
    for (const currentKey in eventHandlersRef.current) {
      ...
      mapInstance.on(
        eventName,
        eventHandler(key) as MapEventHandler<typeof eventName>
      );
    }
